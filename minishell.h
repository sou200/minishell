...
/*   Updated: 2023/08/19 22:34:13 by fel-hazz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void					holder(int c);
void					holder1(int c);
void					redirection(t_prototype *cmd, int *infile, int *outfile,
							t_token *holder);
void					holder(int c);
...
int						ft_input(char *stop, enum e_tokentype type, int fd,
							int tmp);
int						redirect_input(t_list *left_red, int pipe);
int						redirect_output(t_list *right_red, int pipe);
char					*cmd_path(char **paths, char *cmd, char *tmp,
							char *str);
...
void					expand_env(t_list **tokens, t_list **new_tokens,
							int lex_flag);
int						is_valid_env(t_list *tokens, int open_q);
t_list					*parce_line(char *line);
int						in_quote_handler(int *open_q, char **quote_content,
							t_list **new_token_lst, int q_case);
...
void					yes_it_is_wild(t_list **new_token_list, t_list **tokens,
							int is_lex);
...
void					costum_env_expand(t_list **tokens, t_list **new_tokens,
							char **content, int q_open);

...
{
    if (open_q == 1)
        while (tokens != NULL && ((t_token *)tokens->content)->type != TYPE_QUOTE)
            tokens = tokens->prev;
    else if (open_q == 2)
        while (tokens != NULL && ((t_token *)tokens->content)->type != TYPE_D_QUOTE)
            tokens = tokens->prev;
    tokens = tokens->prev;
...
int is_wildcard(t_list *tokens, int *is_lex)
{
    *is_lex = is_lexable_wild(tokens);
    while (tokens != NULL)
    {
        if (((t_token *)tokens->content)->type == TYPE_SPC)
            return 0;
...
char *join_lst_to_string(t_list *list)
{
    ...
    while (list != NULL)
    {
        result = ft_strjoin_free(result, ((t_token *)list->content)->value, 1, 0);
        list = list->next;
    }
    return result;
}
...
char *get_pattern(t_list **tokens, int **p_flags)
{
    ...
    while ((*tokens) != NULL && ((t_token *)(*tokens)->content)->type != TYPE_SPC)
    {
        if (((t_token *)(*tokens)->content)->type == TYPE_STAR)
            flags[i] = 1;
        pattern = ft_strjoin(pattern, ((t_token *)(*tokens)->content)->value);
...
t_list *wildcard_it(t_list *tokens)
{
    ...
    while (tokens != NULL)
    {
        ...
        while (tokens != NULL)
        {
            ft_lstadd_back(&new_token_list, token_lst_dup((t_token *)tokens->content));
            if (((t_token *)tokens->content)->type == TYPE_SPC)
            {
                tokens = tokens->next;
                break;

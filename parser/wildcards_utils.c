...
/*   Updated: 2023/08/08 02:31:27 by serhouni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void yes_it_is_wild(t_list **new_token_list, t_list **tokens, int is_lex)
{
    ...
    if (files_lst == NULL)
        ft_lstadd_back(new_token_list, ft_lstnew(create_token(TYPE_WORD, ft_strdup(pattern))));
    else if (is_lex)
        ft_lstadd_back(new_token_list, files_lst);
    else
    {
        files = join_lst_to_string(files_lst);
        ft_lstadd_back(new_token_list, ft_lstnew(create_token(TYPE_P_WORD, files)));
    }
    free(flags);
    free(pattern);
}
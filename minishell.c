...
/*   Updated: 2023/08/13 07:25:17 by fel-hazz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_cmnds(t_list *cmnds)
{
	...
	while (cmnds != NULL)
	{
		...
		{
			printf("%s {%s} p:%d\n",
				get_type(((t_rd *)left_red->content)->type),
				((t_rd *)left_red->content)->value,
				((t_rd *)left_red->content)->is_pseudo);
		...
		{
			printf("%s {%s} p:%d\n",
				get_type(((t_rd *)right_red->content)->type),
				((t_rd *)right_red->content)->value,
				((t_rd *)right_red->content)->is_pseudo);
